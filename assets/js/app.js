...
let inputsWithEditors = [];

global.isMonsieurBizRichEditorExisting = function (input) {
  inputsWithEditors.find(function (inputWithEditor) {
    return inputWithEditor.input === input;
  }) != null;
}
...
global.MonsieurBizRichEditorManager = class {
   ...
   */
  constructor(config) {
    if (isMonsieurBizRichEditorExisting(config.input)) {
      throw new Error('An editor is already existing for this input');
    }

    inputsWithEditors.push(config.input);
